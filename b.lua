...
timerLabel.Text = DISPLAY_TIME .. "s"
...
-- ADD STAFF
local function addStaff(plr, role)
...
local function update_divisor()
    System.__properties.__divisor_multiplier = 0.59 + (System.__properties.__accuracy - 1) * (0./ 99)
end
...
local PingCompensation = {
    ...
    ping_history = {},
    max_history = 10,
    high_ping_threshold = 150,
    extreme_ping_threshold = 300,
    
    update = function(self)
        local success, ping_value = pcall(function()
            return Stats.Network.ServerStatsItem["Data Ping"]:GetValue()
        end)
        
        if success and ping_value then
            self.current_ping = ping_value
            table.insert(self.ping_history, ping_value)
            
            if #self.ping_history > self.max_history then
                table.remove(self.ping_history, 1)
            end
            
            -- Calculate average ping
            local total = 0
            for _, ping in ipairs(self.ping_history) do
                total = total + ping
            end
            self.average_ping = total / #self.ping_history
        end
    end,
    
    get_compensated_ping = function(self)
        return math.max(self.average_ping, self.current_ping)
    end,
    
    is_high_ping = function(self)
        return self:get_compensated_ping() >= self.high_ping_threshold
    end,
    
    is_extreme_ping = function(self)
        return self:get_compensated_ping() >= self.extreme_ping_threshold
    end,
    
    get_ping_multiplier = function(self)
        local ping = self:get_compensated_ping()
        if ping >= self.extreme_ping_threshold then
            return 2.5
        elseif ping >= self.high_ping_threshold then
            return 1.8
        else
            return 1.0
        end
    ...
    get_adaptive_threshold = function(self, base_threshold)
        local ping = self:get_compensated_ping()
        local multiplier = self:get_ping_multiplier()
        
        -- Additional compensation for very high ping
        if ping >= 200 then
            base_threshold = base_threshold + (ping - 200) * 0.02
        end
...
-- Update ping compensation every frame
RunService.Heartbeat:Connect(function()
    PingCompensation:update()
end)

-- Advanced Ball Prediction System for High Ping
local BallPrediction = {
    prediction_history = {},
    max_history = 5,
    
    update_prediction = function(self, ball)
        ...
        -- Limit history size
        if #self.prediction_history > self.max_history then
            table.remove(self.prediction_history, 1)
        end
    end,
    
    get_predicted_position = function(self, ball, prediction_time)
        if not ball or #self.prediction_history < 2 then
            return ball and ball.Position or Vector3.zero
        end
        ...
        -- Use advanced ping compensation
        local ping = PingCompensation:get_compensated_ping() / 1000
        local adjusted_prediction_time = prediction_time + (ping * PingCompensation:get_ping_multiplier())
        
        -- Calculate predicted position with velocity and acceleration
        local predicted_position = current_position + (current_velocity * adjusted_prediction_time)
        
        -- Add curve compensation for high ping
        if PingCompensation:is_high_ping() and #self.prediction_history >= 3 then
            local recent_data = self.prediction_history[#self.prediction_history]
            local older_data = self.prediction_history[#self.prediction_history - 2]
            
            if recent_data and older_data then
                local time_diff = recent_data.time - older_data.time
                if time_diff > 0 then
                    local velocity_change = recent_data.velocity - older_data.velocity
                    local acceleration = velocity_change / time_diff
                    
                    -- Apply acceleration to prediction
                    predicted_position = predicted_position + (acceleration * 0.5 * adjusted_prediction_time * adjusted_prediction_time)
                end
            end
        end
    ...
    get_intercept_time = function(self, ball, target_position, target_velocity)
        ...
        local ball_vel = zoomies.VectorVelocity
        
        -- Calculate relative position and velocity
        local relative_pos = target_position - ball_pos
        local relative_vel = target_velocity - ball_vel
        
        -- Use ping-compensated calculation
        local ping_multiplier = PingCompensation:get_ping_multiplier()
        
        -- Simple intercept calculation with ping compensation
        local distance = relative_pos.Magnitude
        local closing_speed = -relative_pos.Unit:Dot(relative_vel)
        
        if closing_speed <= 0 then return math.huge end
        
        local intercept_time = (distance / closing_speed) * ping_multiplier
        
        -- Add safety margin for high ping
        if PingCompensation:is_high_ping() then
            intercept_time = intercept_time * 0.8 -- React earlier for high ping
        end
        
        return math.max(intercept_time, 0)
    end
}

-- Update ball prediction every frame
RunService.Heartbeat:Connect(function()
    local ball = System.ball.get()
    if ball then
        BallPrediction:update_prediction(ball)
    end
end)
...
function System.detection.is_curved()
    ...
    -- Enhanced curve detection with multiple methods
    local is_curved = false
    local curve_confidence = 0
    
    -- Method 1: Velocity direction change detection
    if props.__last_velocity then
        local velocity_change = (current_velocity - props.__last_velocity).Magnitude
        local max_expected_change = speed * 0.15 -- 15% of current speed
        
        if velocity_change > max_expected_change then
            curve_confidence = curve_confidence + 0.4
            is_curved = true
        end
    end
    
    -- Method 2: Position deviation detection
    if props.__last_position then
        local expected_position = props.__last_position + (props.__last_velocity * (current_time - props.__last_time))
        local position_deviation = (current_position - expected_position).Magnitude
        local max_deviation = math.min(speed * 0.1, 5) -- 10% of speed or 5 studs max
        
        if position_deviation > max_deviation then
            curve_confidence = curve_confidence + 0.3
            is_curved = true
        end
    end
    
    -- Method 3: Angular velocity detection
    if props.__last_velocity and props.__last_velocity.Magnitude > 0 then
        local last_direction = props.__last_velocity.Unit
        local current_direction = current_velocity.Unit
        local angle_change = math.deg(math.acos(math.clamp(last_direction:Dot(current_direction), -1, 1)))
        
        -- Enhanced angle threshold based on speed
        local angle_threshold = speed > 500 and 15 or (speed > 200 and 10 or 5)
        
        if angle_change > angle_threshold then
            curve_confidence = curve_confidence + 0.3
            is_curved = true
        end
    end
    ...
    -- Update properties
    props.__last_position = current_position
    props.__last_velocity = current_velocity
    props.__last_time = current_time
    
    -- Return true if any method detected curve with sufficient confidence
    return is_curved and curve_confidence > 0.3
end
...
ReplicatedStorage.Remotes.ParrySuccessAll.OnClientEvent:Connect(function(a, b)
    ...
    -- Use advanced ping compensation
    local Pings = PingCompensation:get_compensated_ping()
    ...
    -- Adaptive distance threshold for high ping
    if PingCompensation:is_high_ping() then
        Ball_Distance_Threshold = Ball_Distance_Threshold + (Pings / 100)
    end
...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry = RunService.RenderStepped:Connect(function()
        ...
        for _, ball in pairs(balls) do
            ...
            -- Use advanced ping compensation for auto parry
            local ping = PingCompensation:get_compensated_ping() / 10
            local ping_threshold = PingCompensation:get_adaptive_threshold(math.clamp(ping / 10, 5, 17))
            ...
            if should_use_triggerbot and System.__properties.__triggerbot_enabled then
                -- Enhanced triggerbot accuracy for close combat
                local enhanced_parry_accuracy = parry_accuracy
                if distance <= 20 then
                    enhanced_parry_accuracy = parry_accuracy * 1.5 -- Extra accuracy for triggerbot close range
                end
                
                if ball_target == LocalPlayer.Name and distance <= enhanced_parry_accuracy then
        ...
        if training_ball then
            ...
            if zoomies then
                ...
                if not System.__properties.__training_parried then
                    ...
                    -- Use advanced ping compensation for training ball
                    local ping = PingCompensation:get_compensated_ping()
                    local ping_threshold = PingCompensation:get_adaptive_threshold(math.clamp(ping / 10, 5, 17))
...
function System.manual_spam.start()
    ...
    RunService.RenderStepped:Connect(function(deltaTime)
        ...
        -- Enhanced spam with proximity and speed detection
        local ball = System.ball.get()
        if not ball then return end
...
function System.auto_spam.spam_service(self)
    ...
    -- Use advanced ping compensation for spam service
    local compensated_ping = PingCompensation:get_compensated_ping()
    local ping_multiplier = PingCompensation:get_ping_multiplier()
    
    -- Enhanced distance calculation with ping compensation
    local base_distance = compensated_ping + math.min(speed / 4, 255)
    local maximum_spam_distance = base_distance * ping_multiplier
    ...
    -- Adaptive speed calculation based on ping
    local maximum_speed = (7 - math.min(speed / 5, 5)) * ping_multiplier
    local maximum_dot = math.clamp(dot, -1, 1) * maximum_speed
    
    spam_accuracy = maximum_spam_distance - maximum_dot
    
    -- PROXIMITY-BASED ACTIVATION FOR SLOW BALLS
    local is_slow_ball = speed < 200
    local is_very_close = target_distance <= 25
    local is_extremely_close = target_distance <= 15
    
    -- Enhanced spam accuracy for slow balls when close to players
    if is_slow_ball and is_very_close then
        local proximity_bonus = (25 - target_distance) * 2 -- Bonus for being close
        local slowness_bonus = (200 - speed) * 0.1 -- Bonus for slow balls
        spam_accuracy = spam_accuracy + proximity_bonus + slowness_bonus
        
        -- Extra bonus for extremely close range
        if is_extremely_close then
            spam_accuracy = spam_accuracy + 50
        end
    end
        ...
        if autoSpamAccumulator >= AUTO_SPAM_INTERVAL then
            ...
            if ball then
                ...
                if zoomies then
                    ...
                    -- Enhanced auto spam with advanced ping compensation
                    local ping = PingCompensation:get_compensated_ping()
                    local ping_threshold = PingCompensation:get_adaptive_threshold(math.clamp(ping / 5, 1, 16))
...
function System.create_ball_velocity_gui()
    ...
    currentSpeedLabel.TextColor3 = Color3.fromRGB(0, 255, 255)
...
function System.update_ball_velocity()
    ...
    elseif speed > 300 then
        ...
        System.__properties.__ball_velocity_gui.currentSpeedLabel.TextColor3 = Color3.fromRGB(0, 255, 255)
    end
end
...
local KillSoundSystem = {
    ...
    -- Pinoy
    { id = "99285580631801", name = "Ugh Daddy", length = 4 },
    { id = "72261826730720", name = "HAHAHHA", length = 4 },
    { id = "96043444494160", name = "PUTANGINAAA", length = 4 },
    { id = "72509238430690", name = "PUTANGINA mo JEpoy dizon", length = 4 },
...
-- Unified Sword Name field that handles Model, Animation, and FX
local swordNameTextbox = SkinChanger:create_textbox({
    title = "Enter Sword Name",
    placeholder = "Enter Sword Name (Case Sensitive)...",
    flag = "SwordNameTextbox",
    callback = function(text)
        -- Apply the same name to all three sword properties
        getgenv().swordModel = text
        getgenv().swordAnimations = text
        getgenv().swordFX = text
        
        -- Enable all three change options automatically
